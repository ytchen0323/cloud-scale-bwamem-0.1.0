...
object BWAMemWorker1Batched {
    ...
    */
    def bwaMemWorker1Batched(opt: MemOptType, //BWA MEM options
        ...
        val readArray = new Array[Array[Byte]](numOfReads)
            ...
            var i = 0
            while (i < numOfReads) {
                readArray(i) = (new String(seqArray(i).getSeq.array))
                    .toCharArray.map(locus => locusEncode(locus))
        ...
        i = 0
        while (i < numOfReads) {
            chainsFilteredArray(i) = memChainFilter(opt, generateChains(opt,
                        bwt, bns.l_pac, lenArray(i), readArray(i))) 
            i = i+1;
        }
        ...
        while (i < numOfReads) {
            if (chainsFilteredArray(i) == null) {
                ...
                while (j < chainsFilteredArray(i).length) {
                    preResultsOfSW(i)(j)= calPreResultsOfSW(opt, bns.l_pac, pac,
                            lenArray(i), readArray(i), chainsFilteredArray(i)(j))
                    j = j+1
                }
                numOfSeedsArray(i) = 0
                ...
                if (debugLevel == 1) {
                  println("Finished the calculation of pre-results of Smith-Waterman")
        ...
        memChainToAlnBatched(opt, bns.l_pac, pac, lenArray, readArray,
                numOfReads, preResultsOfSW, chainsFilteredArray, regArrays,
                runOnFPGA, threshold, isEnd0, seqArray, outputStream, partitionId)
    }
  ...
  def pairEndBwaMemWorker1Batched(opt: MemOptType, //BWA MEM options
    ...
    bwaMemWorker1Batched(opt, bwt, bns, pac, pes, seqArray0,
            numOfReads, runOnFPGA, threshold, true, outputStream, partitionId)
    bwaMemWorker1Batched(opt, bwt, bns, pac, pes, seqArray1,
            numOfReads, runOnFPGA, threshold, false, outputStream, partitionId)
  }
}
